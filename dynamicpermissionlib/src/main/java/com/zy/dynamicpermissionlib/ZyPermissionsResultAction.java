 ...
 */
public abstract class ZyPermissionsResultAction {

    private static final String TAG = ZyPermissionsResultAction.class.getSimpleName();
     ...
     * Default Constructor
     */
    public ZyPermissionsResultAction() {}

    /**
     * Alternate Constructor. Pass the looper you wish the ZyPermissionsResultAction
    ...
    public ZyPermissionsResultAction(@NonNull Looper looper) {mLooper = looper;}

    /**
     * Called inside of {@link android.app.Activity#onRequestPermissionsResult(int, String[], int[])}
     *
     * @param requestCode
     * @param permissions
     * @param grantResults
     ...
     * affect whether the ZyPermissionsResultAction should continue
     ...
     *                   Android version
     * @return return true if the ZyPermissionsResultAction should
     * ignore the lack of the permission and proceed with exection
     * or false if the ZyPermissionsResultAction should treat the
    ...
    protected synchronized final boolean onResult(final @NonNull String permission, int result) {
        if (result == PackageManager.PERMISSION_GRANTED) {
            return onResult(permission, ZyPermissionsEnum.GRANTED);
        } else {
            return onResult(permission, ZyPermissionsEnum.DENIED);
        }
    ...
    @SuppressWarnings("WeakerAccess")
    @CallSuper
    protected synchronized final boolean onResult(final @NonNull String permission, ZyPermissionsEnum result) {
        mPermissions.remove(permission);
        if (result == ZyPermissionsEnum.GRANTED) {
        ...
        } else if (result == ZyPermissionsEnum.DENIED) {
            new Handler(mLooper).post(new Runnable() {
                @Override
                public void run() {
                    onDenied(permission);
            ...
            return true;
        } else if (result == ZyPermissionsEnum.NOT_FOUND) {
    ...
    /**
     * This method registers the ZyPermissionsResultAction object for the specified permissions
     * so that it will know which permissions to look for changes to. The ZyPermissionsResultAction
